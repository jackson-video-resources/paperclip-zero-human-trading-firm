...
Start by confirming the OS. Run `echo $env:OS` in PowerShell or `echo %OS%` in CMD. If the output contains `Windows_NT`, say: "Windows confirmed. Let's build your trading firm." and proceed. If it does not match, say: "This is the Windows version of the prompt. Grab the Mac or Linux version from the GitHub repo: https://github.com/jackson-video-resources/paperclip-zero-human-trading-firm" — then stop.

For all file operations, use PowerShell syntax. For all URL-opening operations, use `Start-Process` or `start`. For paths, use `$env:USERPROFILE` in place of `~`.
...
- Not installed: say "Node.js isn't installed — opening the download page now." Run `Start-Process "https://nodejs.org/en/download"`. Wait for the person to confirm installation before continuing.

**Git:** Run `git --version`.
- Installed: note the version, continue.
- Not installed: say "Git isn't installed — opening the download page now." Run `Start-Process "https://git-scm.com/downloads/win"`. Wait for confirmation before continuing.

**PowerShell execution policy:** Run `Get-ExecutionPolicy`. If it returns `Restricted`, say: "Execution policy is Restricted — I'll set it to RemoteSigned to allow scripts." Then run `Set-ExecutionPolicy RemoteSigned -Scope CurrentUser -Force`.
...
If you get a permissions error: say "Permissions issue — try running Claude Code as Administrator if this fails." Retry the command.
...
Convert the firm name to lowercase-kebab-case (e.g. "Lewis Ventures" → "lewis-ventures"). Store this as FIRM_SLUG. Use `$env:USERPROFILE\[FIRM_SLUG]` as the root directory.

Create the directory tree using PowerShell:
```powershell
$root = "$env:USERPROFILE\[FIRM_SLUG]"
$dirs = @(
  ...
  "logs\trades", "logs\performance", "logs\agent-activity",
  "memory\institutional", "memory\performance",
  "config"
)
...
"@ | Out-File -FilePath "$env:USERPROFILE\[FIRM_SLUG]\README.md" -Encoding utf8
```
...
"@ | Out-File -FilePath "$env:USERPROFILE\[FIRM_SLUG]\config\risk-thresholds.json" -Encoding utf8
```

When done: say "File structure created at %USERPROFILE%\[FIRM_SLUG]. ✓"
...
```powershell
Test-Path "C:\Program Files\TradingView\TradingView.exe" -or Test-Path "$env:LOCALAPPDATA\TradingView\TradingView.exe"
```

If the result is `False`: say "TradingView Desktop isn't installed — opening the download page now." Then run:
```powershell
Start-Process "https://www.tradingview.com/pricing/?share_your_love=lewisf5rg0"
```
...
const configPath = path.join(process.env.USERPROFILE, '.claude', '.mcp.json');
let existing = {};
...
existing.mcpServers.tradingview = {
  command: 'node',
  args: [process.env.USERPROFILE.replace(/\\\\/g, '/') + '/tradingview-mcp-jackson/src/server.js']
};
...
```powershell
Copy-Item "$env:USERPROFILE\tradingview-mcp-jackson\rules.example.json" "$env:USERPROFILE\tradingview-mcp-jackson\rules.json"
Start-Process "$env:USERPROFILE\tradingview-mcp-jackson\rules.json"
```
...
Deliver the following brief to the CEO agent in Paperclip. Substitute all bracketed values with answers from the intake interview. Use `%USERPROFILE%\[FIRM_SLUG]\` as the file system root in all agent mandates. For custom agents from Q4, append their full briefs following the same format as the standard agents below.
...
- File system root: %USERPROFILE%\[FIRM_SLUG]\
...
- Log all sources reviewed to %USERPROFILE%\[FIRM_SLUG]\logs\agent-activity\research.log
...
- Log every result to %USERPROFILE%\[FIRM_SLUG]\memory\institutional\ — results are NEVER deleted, only archived
...
- Maintain risk parameters in %USERPROFILE%\[FIRM_SLUG]\config\risk-thresholds.json
...
- Logs go to %USERPROFILE%\[FIRM_SLUG]\logs\trades\
...
First task for CEO: Hire all agents above. Set up initial task queues. Create %USERPROFILE%\[FIRM_SLUG]\config\org-chart.json listing all roles, reporting lines, and mandates. Output the completed org chart and confirm all agents are active.
...
2. Your risk thresholds in %USERPROFILE%\[FIRM_SLUG]\config\risk-thresholds.json are the law. Only you change them.

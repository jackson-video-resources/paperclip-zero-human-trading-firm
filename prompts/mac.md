...
- **MISSING or INVALID** → Ask: "Paste your Anthropic API key (starts with `sk-ant-`), or type **B** to open console.anthropic.com."
  - B → `open https://console.anthropic.com/settings/keys` — wait.
  - Key pasted → run:
    ```bash
    echo 'export ANTHROPIC_API_KEY="[PASTED_KEY]"' >> ~/.zshrc && export ANTHROPIC_API_KEY="[PASTED_KEY]"
    ```
...
const p=path.join(process.env.HOME,'.claude','.mcp.json');
let c={};try{c=JSON.parse(fs.readFileSync(p,'utf8'))}catch(e){}
c.mcpServers=c.mcpServers||{};
c.mcpServers.tradingview={command:'node',args:[process.env.HOME+'/tradingview-mcp-jackson/src/server.js']};
...
async function main() {
  ...
  // CEO must delegate — and we give it the exact working curl commands.
...
http.get('http://localhost:3100/api/companies', res => {
  ...
  res.on('end', () => {
    ...
    require('child_process').exec('open ' + f);
